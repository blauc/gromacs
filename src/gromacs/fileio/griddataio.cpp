...
class MrcFile::Impl
{
    public:
        ...
        void set_grid_stats(const GridDataReal3D &grid_data);
...
std::unique_ptr < IGrid < DIM>> MrcFile::Impl::setGridWithTranslationFromMrcMeta()
{
    ...
    Grid<DIM>                      tmpgrid({{{cell_length[XX], cell_length[YY], cell_length[ZZ]}}}, meta_.extend);
    ...
    GridWithTranslation<DIM>       result({{{cell_length[XX], cell_length[YY], cell_length[ZZ]}}}, meta_.extend, translation);
...
void
MrcFile::Impl::set_grid_stats(const GridDataReal3D &grid_data)
{
...
GridDataReal3D MrcFile::read(std::string filename)
{
    ...
    auto result = GridDataReal3D(std::move(grid));
