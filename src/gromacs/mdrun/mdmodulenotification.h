 ...
 * \tparam MdModuleNotificationBase class to be extended with a notification with CallParameter
 *
   \msc
   wordwraparcs=true,
   hscale="2";
...
/*! \internal
 * \brief Helper struct to avoid spelling out a nested MdModuleNotification
 *        type definition.
 *
 * Instead of
...
{
    /*! \internal \brief Do nothing but be base class of MdModuleNotification.
    ...
    /*! \brief Define a type even if no events are managed, so that code works
     * with MdModuleCallParameterManagement that does not actually manage any event.
     */
    using type = NoCallParameter;
};

/*! \internal \brief Template specialization that assembles the MdModuleNotification type
 *                   by taking off the front of the CallParameter parameter pack.
 *
